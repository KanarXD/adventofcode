    ...
    for y in 0..height {
        for x in 0..width {
            if let Location::Antenna(character) = locations[y as usize][x as usize] {
                ...
                mark_location_antinodes(&mut antinodes, antenna_x, antenna_y, -delta_x, -delta_y);
                mark_location_antinodes(&mut antinodes, x, y, delta_x, delta_y);
            }
        }
    }
}

fn mark_location_antinodes(
    mut antinodes: &mut Vec<Vec<bool>>,
    mut x: isize,
    mut y: isize,
    delta_x: isize,
    ...
    loop {
        x -= delta_x;
        y -= delta_y;
        if mark_location_antinode(&mut antinodes, x, y) {
            break;
        }
    }
}

fn mark_location_antinode(antinodes: &mut Vec<Vec<bool>>, x: isize, y: isize) -> bool {
