...
fn bfs(matrix: &Vec<Vec<Tile>>, start_position: &Position, start_direction: &Direction) -> u64 {
    let mut to_visit: LinkedList<(DirectedPosition, u64, HashSet<Position>)> = LinkedList::new();
    let mut ending_paths: HashSet<Position> = HashSet::new();
    ending_paths.insert(start_position.clone());
    to_visit.push_back((
        DirectedPosition {
    ...
    while to_visit.len() > 0 {
        ...
        let mut new_to_visit: LinkedList<(DirectedPosition, u64, HashSet<Position>)> =
            LinkedList::new();
        for (directed_position, score, path) in to_visit.iter() {
            ...
            if let Some(tile) = get_tile(matrix, &position) {
                ...
                if tile == Tile::End {
                    ...
                    // println!("path: {path:?}");
                    // path.clone_into(&mut ending_paths);

                    if result > *score {
                        path.clone_into(&mut ending_paths);
                        result = *score;
                    } else if result == *score {
                        ending_paths.extend(path);
                    }
                    continue;
                }
                ...
                .for_each(|checked_direction| {
                    ...
                    let checked_directed_position = DirectedPosition {
                        position: checked_position.clone(),
                        direction: checked_direction,
                    };
                    let mut new_path = HashSet::new();
                    path.clone_into(&mut new_path);
                    new_path.insert(checked_position);
                    new_to_visit.push_back((checked_directed_position, checked_score, new_path));
                });
            }
        }
    ...
    // println!("cache: {:?}", cache);
    print_matrix(matrix, &ending_paths);

    ending_paths.len() as u64 + 1
}

fn print_matrix(matrix: &Vec<Vec<Tile>>, paths: &HashSet<Position>) {
