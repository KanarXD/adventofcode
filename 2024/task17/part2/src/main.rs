...
// const START_INDEX: u64 = 100_000_000_000_000;
const START_INDEX: u64 = 727055768;
// const START_INDEX: u64 = 0;

const LOG_OUTPUT: &[u64] = &[7, 4, 4, 0, 3, 5, 5, 3, 0];
const ALWAYS_LOG: bool = true;
...
struct Instruction {
    opcode: u64,
    action: fn(&mut ComputerState) -> Option<u64>,
}
...
const INSTRUCTIONS: &[&Instruction] = &[
    &Instruction {
        ...
        action: |computer_state| {
            ...
            let denominator = 2_u64.pow(computer_state.combo_operand() as u32);
...
fn process_data(computer: &Computer) -> u64 {
    println!("desired output={:?}", computer.program);
    let mut output_suffix: Vec<u64> = vec![];
    let mut last_start = 0;
    for &num in computer.program.iter().rev() {
        output_suffix.push(num);
        let mut output_suffix_reversed = output_suffix.clone();
        output_suffix_reversed.reverse();
        let first_index = find_index(computer, last_start * 8, &output_suffix_reversed);
        last_start = first_index;
    }
    last_start
}

fn find_index(computer: &Computer, start_i: u64, output_suffix: &Vec<u64>) -> u64 {
    println!("start_i={start_i}, output_suffix={:?}", output_suffix);
    let mut i: u64 = start_i;
    loop {
        let mut test_computer = computer.clone();
        test_computer.registers.a = i;

        let output = run_computer(test_computer);

        if output == computer.program {
            return i;
        }
        if output.ends_with(output_suffix) {
            println!("start_i={start_i}, output_suffix={output_suffix:?}, found at i={i}");
            return i;
        }
...
fn parse_lines(data: String) -> Computer {
    ...
    let register_values: Vec<u64> = registers_string
        .split("\n")
        .enumerate()
...
fn run_computer(computer: Computer) -> Vec<u64> {
...
fn run_command(mut computer_state: &mut ComputerState) -> Option<u64> {
