...
impl ComputerState {
    fn increase_pointer(&mut self) -> bool {
        self.pointer += 2;
    ...
    fn combo_operand(&self) -> i32 {
        match self.literal_operand() {
            0..=3 => self.literal_operand() as i32,
...
struct Instruction {
    opcode: u32,
    action: fn(&mut ComputerState) -> Option<i32>,
}

const INSTRUCTIONS: &[&Instruction] = &[
    &Instruction {
        opcode: 0,
        action: |computer_state| Some(0),
    },
    &Instruction {
        opcode: 1,
        action: |computer_state| Some(1),
    },
    &Instruction {
        opcode: 2,
        action: |computer_state| None,
    },
    &Instruction {
        opcode: 3,
        action: |computer_state| None,
...
fn parse_lines(data: String) -> Computer {
    ...
    let register_values = registers_string
        .split("\n")
        .enumerate()
...
fn process_data(computer: &Computer) -> String {
    ...
    let mut output_values: Vec<i32> = vec![];
    ...
    loop {
        ...
        if computer_state.increase_pointer() {
            break;
        }
    }
...
fn run_command(computer_state: &mut ComputerState) -> Option<i32> {
    let instruction = INSTRUCTIONS
        .into_iter()
        .find(|&instruction| instruction.opcode == computer_state.opcode())
        .unwrap();
    (instruction.action)(computer_state)
}
